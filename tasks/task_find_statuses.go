			...
			for {
				...
				if inMaintLine && tt == html.TextToken {
					...
					if strings.Contains(t.Data, "ldst_strftime") {
						...
						row := m.GetRawPGXPool().QueryRow(context.Background(), "select id from ls_status where id = $1 and region = $2", hash, v)
						var bid string
						if err := row.Scan(&bid); err != nil && err == pgx.ErrNoRows {
							...
							_, err = m.GetRawPGXPool().Exec(context.Background(), "insert into ls_status (id, region, title, uri, square_edit, status_body)"+
