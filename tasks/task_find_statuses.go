...
func SetupGetStatusPages(m milieu.Milieu) func() {
	return func() {
			...
			for {
				...
				if inMaintLine && tt == html.TextToken {
					...
					if strings.Contains(t.Data, "ldst_strftime") {
						...
						row := milieu.GetRawPGXPool().QueryRow(context.Background(), "select id from ls_status where id = $1 and region = $2", hash, v)
						var bid string
						if err := row.Scan(&bid); err != nil && err == pgx.ErrNoRows {
							...
							_, err = milieu.GetRawPGXPool().Exec(context.Background(), "insert into ls_status (id, region, title, uri, square_edit, status_body)"+
