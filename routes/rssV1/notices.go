...
func GetNoticeForLang(region support.Regions) func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		rows, err := milieu.GetRawPGXPool().Query(context.Background(), "Select id, title, uri, square_edit, notice_body from ls_notices where region = $1 order by date_found desc limit 10", region)
		if err != nil {
